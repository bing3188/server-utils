  ...
  amqplib
    ...
    .then(conn => {
      conn
        .createChannel()
        .then(ch => {
          return ch.assertQueue(qname, { durable: true }).then(() => {
            return ch.consume(
              qname,
              async msg => {
                if (msg !== null) {
...
const sendToQueue = async (queue, url, message, delay) => {
  return new Promise((resolve, reject) => {
        ...
        conn
          ...
          .then(function(ch) {
            if (delay) {
              ...
              ch.bindQueue(queue, exchange, delayQueue)
              ch.publish(exchange, queue, Buffer.from(JSON.stringify({ url: url, message: message })), { headers: { 'x-delay': delay } })
            } else {
